...
const TABS = [
  ...
  { id: 'intel', label: 'Intel Gaming PCs', icon: Cpu },
  { id: 'ryzen', label: 'AMD Ryzen Builds', icon: Gamepad2 },
  { id: 'laptops', label: 'Gaming Laptops', icon: Laptop },
  { id: 'components', label: 'Components & GPUs', icon: Sliders },
]
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between border-b border-border/80 pb-4">
        <div>
          <span className="inline-flex items-center gap-1.5 text-xs font-extrabold uppercase tracking-wider text-volt">
            <Sparkles className="h-3.5 w-3.5" /> High Performance Setups
          </span>
          <h2 className="font-display text-2xl font-black md:text-3xl tracking-tight mt-0.5">
            Featured PCs, Laptops & Components
          </h2>
        </div>
      ...
      <div className="mt-8 flex justify-center">
        ...
        >
          View Full Store Catalog ({products.length} Products) <ArrowRight className="h-4 w-4" />
        </Link>
      </div>
